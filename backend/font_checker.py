...
def verify_font_sizes(
    ...
    try:
        ...
        # Tesseract binary not available (e.g. on Vercel) — return a graceful skip
        print(f"[Font Checker] Tesseract unavailable: {str(e)}")
        return {
            "status": "SKIPPED",
            ...
            "font_checks": [],
            "rule_7_details": (
                f"Rule 7 font height check requires Tesseract OCR which is not "
                f"available in this environment. Required height: {req_height_mm}mm."
            )
        }
